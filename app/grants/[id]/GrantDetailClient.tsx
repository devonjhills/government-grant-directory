...
import { getOpportunityTypeConfig, formatOpportunityMetrics, getSuccessTerminology } from "@/app/lib/opportunity-types";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Separator } from "@/components/ui/separator";
import { Skeleton } from "@/components/ui/skeleton";
import { ArrowLeft, Calendar, DollarSign, Building2, Hash, Mail, Phone, Users, FileText, Info, ExternalLink, Target, Clock, BarChart3, CheckCircle2, TrendingUp } from "lucide-react";
import {
  Card,
  CardContent,
  CardDescription,
...
export default function GrantDetailClient({ params }: GrantDetailPageProps) {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (opportunity) {
          ...
          try {
            const enhanced = await enhancedDataEnrichment.enhanceOpportunityWithIntelligence(opportunity);
            setIntelligenceData({
  ...
  const jsonLdData = {
    ...
    "@type": opportunityData.type === 'grant' ? "GovernmentGrant" : "GovernmentService",
    name: getMeaningfulTitle(opportunityData),
    description: opportunityData.description,
    url: typeof window !== "undefined" ? window.location.href : `/grants/${opportunityData.id}`,
    provider: {
    ...
    offers: {
      "@type": "Offer",
      priceRange: opportunityData.amount > 0 ? `$${opportunityData.amount.toLocaleString()}` : "Varies",
      availability: opportunityData.opportunityStatus === 'open' ? "InStock" : "OutOfStock",
    },
  };
        ...
        <div className="space-clean-loose">
            ...
            <CardHeader className="pb-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <Badge className={getStatusColor(opportunityData.opportunityStatus || "Unknown")}>
                ...
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  {opportunityData.agencyCode && (
                    <Badge variant="outline" className="text-xs">{opportunityData.agencyCode}</Badge>
                  )}
                </div>
          ...
          <div className="space-y-6">
              ...
              {intelligenceData?.intelligence && (
                <Card className={`border-l-4 ${typeConfig.colorScheme.border} ${typeConfig.colorScheme.background}/30`}>
                  <CardHeader>
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      <div className="text-center">
                        <div className={`text-6xl font-bold ${typeConfig.colorScheme.primary}`}>
                      ...
                      <div className="space-y-3 text-sm bg-white/80 p-4 rounded-lg border">
                        <div className="font-semibold text-foreground mb-2">Calculation Breakdown:</div>
                        <div className="flex items-center gap-2">
                          <div className={`w-3 h-3 rounded-full ${typeConfig.colorScheme.background}`}></div>
                          <span>VA baseline: 18% success rate</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className={`w-3 h-3 rounded-full ${typeConfig.colorScheme.background}`}></div>
                          <span>Large infrastructure: +7% (fewer capable applicants)</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className={`w-3 h-3 rounded-full ${typeConfig.colorScheme.background}`}></div>
                          <span>Limited eligibility: +5% (state/tribal only)</span>
                        </div>
                      </div>
                    </div>
                  </CardContent>
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-2">
                      ...
                      <div className="text-xl font-semibold">
                        {formatCurrency(opportunityData.awardCeiling || opportunityData.amount)}
                      </div>
                  ...
                  <Card>
                    <CardContent className="p-4">
                      <div className="space-y-2">
                        ...
                        <div className="text-sm text-muted-foreground">
                          {intelligenceData.intelligence.competitionLevel} competition
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                ...
                <Card className="lg:col-span-3">
                  ...
                  <CardContent>
                    <div 
                      ...
                      dangerouslySetInnerHTML={{ __html: opportunityData.description }}
                    />
                  </CardContent>
                </Card>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      <div className="text-sm">
                        ...
                        <div className="font-medium">{formatGrantDetailDate(opportunityData.postedDate)}</div>
                      </div>
                      {opportunityData.cfda && opportunityData.cfda.length > 0 && (
                        <div className="text-sm">
                          <div className="text-muted-foreground">CFDA Program</div>
                          <div className="font-mono text-xs">{opportunityData.cfda[0]}</div>
                        </div>
                      )}
                      <div className="text-sm">
                        <div className="text-muted-foreground">Source</div>
                        <div className="font-medium">{opportunityData.sourceAPI}</div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-2">
                        {opportunityData.contactInfo.name && (
                          <div className="font-medium text-sm">{opportunityData.contactInfo.name}</div>
                        )}
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div 
                      className="prose prose-sm max-w-none leading-relaxed"
                      dangerouslySetInnerHTML={{ __html: opportunityData.eligibilityCriteria }}
                    />
                    {opportunityData.applicantTypes && opportunityData.applicantTypes.length > 0 && (
                      <div>
                        <h4 className="font-medium mb-3 text-sm">Eligible Applicants</h4>
                  ...
                  <CardContent>
                    {intelligenceLoading ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-muted-foreground mt-3 text-sm">Loading analysis...</p>
                      </div>
                      ...
                      <p className="text-muted-foreground text-center py-8 text-sm">Analysis not available</p>
                    )}
                  </CardContent>
                </Card>
              )}
