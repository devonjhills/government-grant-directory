...
interface OptimizationStrategy {
  category: 'application' | 'partnerships' | 'budget' | 'timeline' | 'compliance';
...
export const SuccessOptimizationTool: React.FC<SuccessOptimizationToolProps> = ({
  opportunity,
  readinessScore = 65,
  userProfile
}) => {
  const [selectedStrategies, setSelectedStrategies] = useState<string[]>([]);
  const [optimizationPlan, setOptimizationPlan] = useState<SuccessOptimizationPlan | null>(null);
  ...
  const generateOptimizationStrategies = (): OptimizationStrategy[] => {
    ...
    if (readinessScore < 80) {
      strategies.push({
        ...
        description: 'Develop a compelling, evidence-based project narrative that clearly demonstrates impact and feasibility.',
        impact: 9,
        effort: 7,
        timeline: '2-3 weeks',
        actionItems: [
    ...
    if (opportunity.amount > 500000 || opportunity.categories?.includes('Research & Development')) {
      strategies.push({
        category: 'partnerships',
        priority: 'high',
        title: 'Strategic Partnership Development',
        description: 'Form strategic partnerships to strengthen technical capabilities and increase competitive advantage.',
        impact: 8,
        effort: 6,
        timeline: '3-6 weeks',
        actionItems: [
    ...
    strategies.push({
      ...
      description: 'Develop a detailed, well-justified budget that maximizes impact while demonstrating cost-effectiveness.',
      impact: 7,
      effort: 5,
      timeline: '1-2 weeks',
      actionItems: [
    ...
    if (opportunity.deadline) {
      ...
      if (daysUntilDeadline < 60) {
        strategies.push({
          ...
          description: 'Implement streamlined processes to meet tight deadline while maintaining quality.',
          impact: 6,
          effort: 8,
          timeline: 'Immediate',
          actionItems: [
    ...
    strategies.push({
      ...
      description: 'Ensure 100% compliance with all federal requirements and opportunity-specific criteria.',
      impact: 10,
      effort: 4,
      timeline: '1 week',
      actionItems: [
  ...
  const generateOptimizationPlan = (): SuccessOptimizationPlan => {
    ...
    const immediate = strategies.filter(s => 
      s.timeline.includes('Immediate') || s.timeline.includes('1 week') || s.priority === 'high'
    );
    const shortTerm = strategies.filter(s => 
      s.timeline.includes('2-3 weeks') || s.timeline.includes('1-2 weeks')
    );
    const longTerm = strategies.filter(s => 
      s.timeline.includes('6 weeks') || s.timeline.includes('3-6 weeks')
    );

    const plan: SuccessOptimizationPlan = {
      opportunityId: opportunity.id,
      overallStrategy: readinessScore > 75 
  ...
  const calculateProbabilityIncrease = (strategies: OptimizationStrategy[]): number => {
        ...
        <Card className="bg-gradient-to-r from-green-50 to-blue-50 border-green-200">
          <CardHeader>
            ...
            <p className="text-gray-600">
              Get personalized strategies to maximize your application success probability.
            </p>
          </CardHeader>
          <CardContent>
      ...
      <Card className="bg-gradient-to-r from-blue-50 to-purple-50 border-blue-200">
        <CardHeader>
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="text-right">
              <div className="text-sm text-gray-600">Potential Success Increase</div>
              <div className="text-2xl font-bold text-green-600">+{optimizationPlan.successProbabilityIncrease}%</div>
            </div>
          </div>
        </CardHeader>
      </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card className="border-red-200 bg-red-50">
                  ...
                  <CardContent className="space-y-3">
                    {optimizationPlan.timeline.immediate.map((strategy, index) => (
                ...
                <Card className="border-yellow-200 bg-yellow-50">
                  ...
                  <CardContent className="space-y-3">
                    {optimizationPlan.timeline.shortTerm.map((strategy, index) => (
                      <div key={index} className="p-3 bg-white rounded border">
                        <div className="flex items-center gap-2 mb-2">
                          {getCategoryIcon(strategy.category)}
                          <h5 className="font-medium text-sm">{strategy.title}</h5>
                ...
                <Card className="border-green-200 bg-green-50">
                  ...
                  <CardContent className="space-y-3">
                    {optimizationPlan.timeline.longTerm.map((strategy, index) => (
                      <div key={index} className="p-3 bg-white rounded border">
                        <div className="flex items-center gap-2 mb-2">
                          {getCategoryIcon(strategy.category)}
                          <h5 className="font-medium text-sm">{strategy.title}</h5>
              ...
              <Card>
                ...
                <CardContent>
                  <ul className="space-y-3">
                    {optimizationPlan.budgetConsiderations.map((consideration, index) => (
