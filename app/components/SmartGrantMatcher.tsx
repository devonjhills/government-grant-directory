...
interface UserProfile {
  organizationType: 'nonprofit' | 'small-business' | 'university' | 'large-business' | 'government';
...
export const SmartGrantMatcher: React.FC<SmartGrantMatcherProps> = ({
  ...
  const [sortBy, setSortBy] = useState<'match' | 'success' | 'amount' | 'competition'>('match');
  ...
  const calculateMatchScore = useCallback((opportunity: Opportunity): { score: number; reasons: string[] } => {
    ...
    score += orgScore;
    if (orgScore >= 20) reasons.push(`Perfect fit for ${profile.organizationType} organizations`);

    // Industry focus matching (20 points)
    const industryMatch = opportunity.categories?.some(cat => 
      profile.industryFocus.some(focus => 
    ...
    if (opportunity.amount >= profile.preferredAmounts.min && opportunity.amount <= profile.preferredAmounts.max) {
      score += 20;
      reasons.push('Amount is within your preferred range');
    } else if (opportunity.amount >= profile.budgetRange.min && opportunity.amount <= profile.budgetRange.max) {
      score += 15;
    ...
    const mockDifficulty = opportunity.amount > 1000000 ? 7 : opportunity.amount > 100000 ? 5 : 3;
    const experienceLevels = { 'beginner': 1, 'intermediate': 5, 'advanced': 7, 'expert': 10 };
    const userLevel = experienceLevels[profile.experienceLevel];
    
    if (mockDifficulty <= userLevel + 2) {
      score += 15;
      if (mockDifficulty <= userLevel) reasons.push('Difficulty level matches your experience');
    ...
    const isHighCompetition = opportunity.amount > 1000000 || 
                             opportunity.agency.includes('NSF') || 
                             opportunity.agency.includes('NIH');
    if (profile.riskTolerance === 'high' && isHighCompetition) {
      score += 10;
      reasons.push('High-value, competitive opportunity matches your risk appetite');
  ...
  const enhancedOpportunities: EnhancedOpportunity[] = useMemo(() => {
    return opportunities.map(opp => {
      ...
      const mockIntelligence: GrantIntelligence = {
        ...
        competitionLevel: opp.amount > 1000000 ? 'High' : opp.amount > 100000 ? 'Medium' : 'Low',
        averageAwardAmount: opp.amount || 250000,
        historicalSuccessRate: Math.floor(Math.random() * 30 + 20), // 20-50%
        typicalApplicationCount: opp.amount > 1000000 ? 200 : opp.amount > 100000 ? 75 : 25,
  ...
  const filteredOpportunities = useMemo(() => {
    let filtered = enhancedOpportunities.filter(opp => opp.matchScore >= filterByMatch);
    
    if (showOnlyRecommended) {
      filtered = filtered.filter(opp => 
        opp.matchScore >= 80 || 
        (opp.intelligence.successProbability > 60 && opp.intelligence.competitionLevel !== 'Very High')
      );
    }
    ...
    return filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'competition':
          const competitionOrder = { 'Low': 4, 'Medium': 3, 'High': 2, 'Very High': 1 };
          return competitionOrder[b.intelligence.competitionLevel] - competitionOrder[a.intelligence.competitionLevel];
        default:
          return b.matchScore - a.matchScore;
      }
    });
  ...
  const getMatchScoreBadge = (score: number) => {
    if (score >= 90) return { text: 'Perfect Match', color: 'bg-green-100 text-green-800' };
    if (score >= 80) return { text: 'Excellent Match', color: 'bg-blue-100 text-blue-800' };
    if (score >= 70) return { text: 'Good Match', color: 'bg-yellow-100 text-yellow-800' };
              ...
              <CardContent className="pt-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-sm font-medium">{opportunity.intelligence.successProbability}%</div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-sm font-medium">{opportunity.intelligence.competitionLevel}</div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-sm font-medium">${(opportunity.amount || 0).toLocaleString()}</div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-sm font-medium">{opportunity.intelligence.reviewTimelineEstimate}</div>
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">
              Try adjusting your matching criteria or expanding your search preferences.
            </p>
