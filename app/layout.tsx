      ...
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `
              // Service Worker temporarily disabled to debug FOUC issues
              // TODO: Re-enable once CSS loading is stable
              
              // Register service worker for offline functionality and caching
              // Delayed registration to avoid interfering with initial page load
              if (false && 'serviceWorker' in navigator && 'caches' in window) {
                window.addEventListener('load', function() {
                  // Wait a bit to ensure CSS is fully loaded
                  setTimeout(() => {
                    navigator.serviceWorker.register('/sw.js')
                      .then(function(registration) {
                        console.log('SW registered: ', registration);
                        
                        // Wait for service worker to be active before requesting sync
                        if ('sync' in window.ServiceWorkerRegistration.prototype) {
                          if (registration.active) {
                            registration.sync.register('background-sync-opportunities').catch(console.error);
                          } else {
              ...
              // Web Vitals monitoring - disabled temporarily to avoid module loading issues
              // TODO: Move to client component or fix module path

              // Critical resources will be loaded on-demand instead of preloaded
              // to avoid unused preload warnings
              
              // Force repaint to ensure CSS is applied (development mode fix)
              document.addEventListener('DOMContentLoaded', function() {
                ...
                // Also try triggering a layout recalculation
                setTimeout(() => {
