...
export class HistoricalDataService {
  ...
  async getAgencyHistoricalData(agencyName: string, years = 5): Promise<HistoricalDataPoint[]> {
    ...
    try {
      ...
      for (let year = currentYear - years; year <= currentYear; year++) {
        ...
        const yearData: HistoricalDataPoint = {
          year,
          totalFunding: response.opportunities.reduce((sum, opp) => sum + (opp.amount || 0), 0),
  ...
  async getIndustryAnalytics(naicsCode: string): Promise<IndustryAnalytics | null> {
    ...
    try {
      ...
      const response = await usaSpendingService.searchAwards({
        ...
        limit: 1000,
        postedAfter: new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] // Last year
      });
      ...
      const totalValue = response.opportunities.reduce((sum, opp) => sum + (opp.amount || 0), 0);
  ...
  async getAgencyAnalytics(agencyName: string): Promise<AgencyAnalytics | null> {
    ...
    if (agencyLower.includes('nsf') || agencyLower.includes('national science foundation')) {
      agencyProfile = {
        ...
        seasonalTrends: [{ quarter: 1, fundingPercent: 20 }, { quarter: 2, fundingPercent: 25 }, { quarter: 3, fundingPercent: 25 }, { quarter: 4, fundingPercent: 30 }],
      };
      ...
      agencyProfile = {
        ...
        seasonalTrends: [{ quarter: 1, fundingPercent: 24 }, { quarter: 2, fundingPercent: 26 }, { quarter: 3, fundingPercent: 22 }, { quarter: 4, fundingPercent: 28 }],
      };
    } else if (agencyLower.includes('sba') || agencyLower.includes('small business')) {
      agencyProfile = {
        ...
        seasonalTrends: [{ quarter: 1, fundingPercent: 22 }, { quarter: 2, fundingPercent: 26 }, { quarter: 3, fundingPercent: 24 }, { quarter: 4, fundingPercent: 28 }],
      };
    } else {
      ...
      agencyProfile = {
        ...
        seasonalTrends: [{ quarter: 1, fundingPercent: 18 }, { quarter: 2, fundingPercent: 22 }, { quarter: 3, fundingPercent: 25 }, { quarter: 4, fundingPercent: 35 }],
      };
    }

    const analytics: AgencyAnalytics = {
      agencyName,
      totalHistoricalFunding: agencyProfile.totalHistoricalFunding || 1000000000,
  ...
  async findSimilarSuccessfulOpportunities(opportunity: Opportunity, limit = 5): Promise<Opportunity[]> {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      yearAgo.setFullYear(yearAgo.getFullYear() - (1 + Math.floor(Math.random() * 3)));
  ...
  private calculateGrowthTrend(opportunities: Opportunity[]): 'increasing' | 'stable' | 'decreasing' {
    // Simple trend calculation based on posting dates
    const now = new Date();
    const sixMonthsAgo = new Date(now.getTime() - 180 * 24 * 60 * 60 * 1000);
    const oneYearAgo = new Date(now.getTime() - 365 * 24 * 60 * 60 * 1000);

    const recent = opportunities.filter(opp => 
      opp.postedDate && new Date(opp.postedDate) > sixMonthsAgo
    ).length;

    const older = opportunities.filter(opp => 
      opp.postedDate && 
  ...
  private estimateCompetitors(opportunities: Opportunity[]): number {
    ...
    const avgAmount = opportunities.length > 0 ? 
      opportunities.reduce((sum, opp) => sum + (opp.amount || 0), 0) / opportunities.length : 0;
