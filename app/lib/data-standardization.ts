...
export const AGENCY_STANDARDIZATION = {
  'Department of Defense': { code: 'DOD', shortName: 'DoD', category: 'Defense' },
  'Department of Health and Human Services': { code: 'HHS', shortName: 'HHS', category: 'Health' },
  'National Science Foundation': { code: 'NSF', shortName: 'NSF', category: 'Science' },
  'Department of Energy': { code: 'DOE', shortName: 'DOE', category: 'Energy' },
  'Department of Education': { code: 'ED', shortName: 'DoEd', category: 'Education' },
  'Environmental Protection Agency': { code: 'EPA', shortName: 'EPA', category: 'Environment' },
  'National Institutes of Health': { code: 'NIH', shortName: 'NIH', category: 'Health' },
  'Small Business Administration': { code: 'SBA', shortName: 'SBA', category: 'Business' },
  'Department of Agriculture': { code: 'USDA', shortName: 'USDA', category: 'Agriculture' },
  'Department of Transportation': { code: 'DOT', shortName: 'DOT', category: 'Transportation' },
  'Department of Commerce': { code: 'DOC', shortName: 'DoC', category: 'Commerce' },
  'Department of Justice': { code: 'DOJ', shortName: 'DOJ', category: 'Justice' },
  'Department of Homeland Security': { code: 'DHS', shortName: 'DHS', category: 'Security' },
  'General Services Administration': { code: 'GSA', shortName: 'GSA', category: 'Administration' },
};
...
export class DataEnrichmentEngine {
  ...
  private locationCache: Map<string, { state?: string; city?: string; country?: string }>;
  ...
  standardizeOpportunity(sourceData: any, sourceType: 'grants.gov' | 'usaspending.gov' | 'state' | 'local'): Opportunity {
    switch (sourceType) {
      case 'grants.gov':
  ...
  private standardizeGeneric(data: any, sourceType: string): Opportunity {
    const standardized: Opportunity = {
      ...
      deadline: this.standardizeDate(data.deadline || data.dueDate || data.closeDate),
      amount: this.extractAmount(data.amount || data.value || data.estimatedValue),
      ...
      categories: this.extractCategories(data.title || '', data.description || ''),
  ...
  private standardizeAgency(agency: string): string {
    ...
    const standardInfo = AGENCY_STANDARDIZATION[normalized as keyof typeof AGENCY_STANDARDIZATION];
  ...
  private standardizeStatus(status: string): string {
    ...
    return STATUS_MAPPING[status.toLowerCase() as keyof typeof STATUS_MAPPING] || status.toLowerCase();
  }

  private standardizeNAICS(naicsCodes: string[]): string[] {
    return naicsCodes.map(code => {
      const sector = code.substring(0, 2);
      return NAICS_CATEGORIES[sector as keyof typeof NAICS_CATEGORIES] || code;
    }).filter(Boolean);
  }
  ...
  private extractCategories(title: string, description: string): string[] {
    ...
    if (/artificial intelligence|machine learning|ai |ml |data science/i.test(content)) {
      categories.add('Artificial Intelligence & Machine Learning');
    }
    if (/cybersecurity|cyber security|information security|data protection/i.test(content)) {
      categories.add('Cybersecurity');
    }
    if (/software|technology|digital|it |information technology/i.test(content)) {
    ...
    if (/infrastructure|construction|building|facility|renovation/i.test(content)) {
      categories.add('Infrastructure & Construction');
    }
    if (/environment|environmental|climate|sustainability|green energy/i.test(content)) {
  ...
  private determineBusinessSize(opportunity: Opportunity): string[] {
    ...
    if (opportunity.setAsideType || 
        /small business|sba|8\(a\)|hubzone|sdvosb|wosb/i.test(opportunity.description)) {
      sizes.push('small-business');
    }

    // Check for large business indicators
    if (opportunity.amount > 10000000) { // $10M+
  ...
  private determineOpportunityType(type?: string, baseType?: string): Opportunity['type'] {
    const typeStr = (type || baseType || '').toLowerCase();
    
    if (typeStr.includes('grant') || typeStr.includes('funding')) return 'grant';
    if (typeStr.includes('contract') || typeStr.includes('procurement')) return 'contract';
    if (typeStr.includes('cooperative')) return 'cooperative_agreement';
    
    return 'other';
  }

  private determineOpportunityTypeFromAward(awardType: string): Opportunity['type'] {
    const type = awardType.toLowerCase();
    
    if (type.includes('grant') || type.includes('assistance')) return 'grant';
    if (type.includes('contract') || type.includes('procurement')) return 'contract';
  ...
  private standardizeSetAside(setAside?: string): string | undefined {
    ...
    return SET_ASIDE_TYPES[setAside.toUpperCase() as keyof typeof SET_ASIDE_TYPES] || setAside;
  }

  private buildIndustryKeywords(): Map<string, string[]> {
    const map = new Map<string, string[]>();
    
    map.set('Technology', ['software', 'ai', 'machine learning', 'cybersecurity', 'cloud', 'data']);
    map.set('Healthcare', ['medical', 'health', 'clinical', 'biomedical', 'pharmaceutical']);
    map.set('Research', ['research', 'study', 'scientific', 'innovation', 'development']);
    map.set('Education', ['education', 'training', 'learning', 'academic', 'curriculum']);
    map.set('Infrastructure', ['construction', 'infrastructure', 'facility', 'building']);
    map.set('Environmental', ['environment', 'climate', 'sustainability', 'green', 'renewable']);
