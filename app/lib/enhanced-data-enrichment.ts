...
export class EnhancedDataEnrichment {
  ...
  async enhanceOpportunityWithIntelligence(opportunity: Opportunity): Promise<
    Opportunity & {
  ...
  private async calculateGrantIntelligence(
    opportunity: Opportunity,
  ): Promise<GrantIntelligence> {
    ...
    const intelligence: GrantIntelligence = {
      ...
      recommendedApplicantProfile:
        await this.generateApplicantProfile(opportunity),
    };
  ...
  private async calculateSuccessProbability(
    ...
    if (opportunity.amount > 0) {
      if (opportunity.amount < 25000)
        successRate = 12; // Small grants very competitive
      else if (opportunity.amount < 100000)
        successRate = 15; // Standard competition
      else if (opportunity.amount < 1000000)
        successRate = 18; // Moderate competition
    ...
    if (agency.includes("nsf"))
      successRate = 11; // NSF published ~11% success rate
    else if (agency.includes("nih"))
      successRate = 14; // NIH ~14% success rate
    else if (agency.includes("doe"))
      successRate = 16; // DOE moderate competition
  ...
  private async assessCompetitionLevel(
    opportunity: Opportunity,
  ): Promise<GrantIntelligence["competitionLevel"]> {
    ...
    if (
      agency.includes("nsf") ||
      agency.includes("national science foundation")
    ) {
  ...
  private async calculateApplicationReadiness(
    opportunity: Opportunity,
    userProfile?: any,
    ...
    readiness.missingRequirements = await this.identifyMissingRequirements(
      opportunity,
      userProfile,
    );
    readiness.recommendedActions = await this.generateRecommendedActions(
      opportunity,
      userProfile,
    );
    readiness.similarOpportunities =
      await this.findSimilarOpportunities(opportunity);
  ...
  private async getAverageAwardAmount(
    opportunity: Opportunity,
  ): Promise<number> {
  ...
  private async estimateApplicationCount(
    opportunity: Opportunity,
  ): Promise<number> {
  ...
  private async estimateReviewTimeline(
    opportunity: Opportunity,
  ): Promise<string> {
  ...
  private async calculateDifficultyScore(
    opportunity: Opportunity,
  ): Promise<number> {
    ...
    if (opportunity.categories.includes("Research & Development"))
      difficulty += 2;
    if (opportunity.categories.includes("Information Technology"))
      difficulty += 1;
  ...
  private async findSimilarSuccesses(
    ...
    try {
      ...
      const similarOps =
        await historicalDataService.findSimilarSuccessfulOpportunities(
          opportunity,
          3,
        );
      return similarOps.map(
        (opp) =>
          `${opp.agency}: ${opp.title} (${opp.postedDate?.substring(0, 4) || "Recent"})`,
      );
    } catch (error) {
  ...
  private async generateApplicantProfile(
    opportunity: Opportunity,
  ): Promise<string> {
    // Generate recommended applicant characteristics
    const profiles: { [key: string]: string } = {
      grant:
        "Non-profit organizations, educational institutions, or research organizations with 3+ years experience and demonstrated track record in the field",
      contract:
        "Established businesses with relevant industry experience, appropriate certifications, and proven delivery capabilities",
      cooperative_agreement:
        "Organizations with strong collaborative capabilities and community partnerships",
    };

    return (
      profiles[opportunity.type] ||
      "Organizations with relevant experience and capacity to execute proposed project"
    );
  }
  ...
  private async identifyMissingRequirements(
    ...
    if (!userProfile?.samRegistration)
      requirements.push("SAM.gov Registration");
    ...
    if (
      ...
      !userProfile?.researchExperience
    ) {
  ...
  private async findSimilarOpportunities(
    opportunity: Opportunity,
  ): Promise<string[]> {
