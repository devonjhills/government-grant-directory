...
export async function POST(request: NextRequest) {
  try {
    ...
    const opportunitiesResponse = await unifiedOpportunityService.searchAllOpportunities({
      ...searchParams,
    ...
    const matchedOpportunities = await Promise.all(
      opportunitiesResponse.opportunities.map(async (opportunity) => {
        ...
        try {
          ...
          console.log('Failed to get intelligence for opportunity:', opportunity.id);
        }
...
function calculateMatchScore(opportunity: any, profile: UserProfile): { score: number; reasons: string[] } {
  let score = 0;
