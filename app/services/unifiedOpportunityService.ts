...
export class UnifiedOpportunityService {
  ...
  async searchAllOpportunities(params: OpportunitySearchParams = {}): Promise<OpportunitySearchResponse> {
    ...
    try {
      ...
      const sortedOpportunities = this.sortOpportunities(allOpportunities, params);
  ...
  async searchGrants(params: OpportunitySearchParams): Promise<{
    ...
    try {
      const grantsParams = {
        ...
        oppStatuses: params.status?.includes('open') ? 'posted|forecasted' : 'posted',
      };
  ...
  async getClosingSoonOpportunities(daysAhead = 30, limit = 20): Promise<Opportunity[]> {
    ...
    return result.opportunities.filter(opp => opp.deadline && new Date(opp.deadline) > new Date());
  }

  // Get opportunities by agency
  async getOpportunitiesByAgency(agencyCode: string, limit = 20): Promise<Opportunity[]> {
    const params: OpportunitySearchParams = {
      agencies: [agencyCode],
      limit,
      sortBy: 'posted_date',
  ...
  async getOpportunitiesByIndustry(naicsCode: string, limit = 20): Promise<Opportunity[]> {
    const params: OpportunitySearchParams = {
  ...
  async getSetAsideOpportunities(setAsideType: string, limit = 20): Promise<Opportunity[]> {
    const params: OpportunitySearchParams = {
  ...
  async getOpportunityStats(): Promise<{
    ...
    try {
      ...
      const recentParams: OpportunitySearchParams = {
        postedAfter: new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Last 90 days
      ...
      const stats = {
        totalActive: result.opportunities.filter(opp => opp.opportunityStatus === 'open').length,
        totalValue: result.opportunities.reduce((sum, opp) => sum + (opp.amount || 0), 0),
  ...
  private sortOpportunities(opportunities: Opportunity[], params: OpportunitySearchParams): Opportunity[] {
    ...
    return opportunities.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'amount':
          ...
          break;
        case 'deadline':
          comparison = new Date(a.deadline || '9999-12-31').getTime() - 
                      new Date(b.deadline || '9999-12-31').getTime();
          break;
        case 'posted_date':
          comparison = new Date(a.postedDate || '1900-01-01').getTime() - 
  ...
  private aggregateBy(opportunities: Opportunity[], field: keyof Opportunity): Record<string, number> {
  ...
  async healthCheck(): Promise<Record<string, { status: 'ok' | 'error'; message: string }>> {
    const [grantsHealth, usaSpendingHealth] = await Promise.allSettled([
      Promise.resolve({ status: 'ok' as const, message: 'Grants.gov service available' }), // Would implement in grantsGovService
      usaSpendingService.healthCheck(),
    ]);

    return {
      'grants.gov': grantsHealth.status === 'fulfilled' ? grantsHealth.value : { status: 'error', message: 'Health check failed' },
      'usaspending.gov': usaSpendingHealth.status === 'fulfilled' ? usaSpendingHealth.value : { status: 'error', message: 'Health check failed' },
    };
  }
}
...
export const getClosingSoonOpportunities = (daysAhead?: number, limit?: number) => 
