...
export class USASpendingService {
  ...
  async searchAwards(params: OpportunitySearchParams = {}): Promise<{
    ...
    try {
      ...
      const cacheKey = CacheKeys.opportunitySearch({
        ...params,
        source: "usaspending",
      });
      ...
      const standardizedOpportunities = await Promise.all(
        data.results.map(async (award) => {
          return dataEnrichment.standardizeOpportunity(
            award,
            "usaspending.gov",
          );
        }),
      ...
      const result = {
        ...
        totalCount:
          data.page_metadata.count || standardizedOpportunities.length,
  ...
  async getAwardDetails(awardId: string): Promise<Opportunity | null> {
    try {
      ...
      const standardized = dataEnrichment.standardizeOpportunity(
        awardData,
        "usaspending.gov",
      );
  ...
  private convertToUSASpendingParams(
    ...
    if (params.postedAfter || params.postedBefore) {
      ...
      const endDate =
        params.postedBefore || new Date().toISOString().split("T")[0];
    ...
    if (params.industryCategories && params.industryCategories.length > 0) {
      const naicsCodes = params.industryCategories.filter((cat) =>
        /^\d{2,6}$/.test(cat),
      );
    ...
    if (params.state && params.state.length > 0) {
      usaParams.filters!.place_of_performance_locations = params.state.map(
        (state) => ({
          country: "USA",
          state: state,
        }),
